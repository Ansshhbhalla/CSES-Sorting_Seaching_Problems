...
int main() {
    ...
    for(int i=0;i<n;i++){
        ...
        auto it = s.lower_bound(a[i]);//ist most closest
        auto it2 = it;
        --it2;//2nd most closest
        ms.erase(ms.find(*it - *it2));
        ms.insert(a[i] - *it2);
        ms.insert(*it - a[i]);
        cout << *--ms.end() << " ";//explained below
        s.insert(a[i]);
    }
    //-------------------------gfg reference--------------------------------//
    // if your multiset contains {3, 5, 7, 9}, then:
    // ms.end() points after 9.
    // --ms.end() points to 9, which is the largest element in the multiset.
    //--------------------------gfg reference------------------------------//
        //vector<ll> v;
