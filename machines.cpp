...
int main() {
    Dracarys
        ...
        //cin>>s1;
        ll a[n],mini=INT_MAX;
        for(int i=0;i<n;i++){cin>>a[i]; mini=min(a[i],mini);}
        //vector<ll> v;
        ...
        //for(int i=0;i<n;i++){}
        ll l=0,r=mini*k,ans=-1;// r is the  time taken by fasten machine that can be the max optimised time by greedy of 1 machine
//         10 10
// 6 6 4 3 4 9 3 2 6 10
        sort(a,a+n);
        while(l<=r){
            ll mid=l+(r-l)/2;    
            ll pro=solve(n,k,a,mid);//calculating number of products 
            if(pro>=k){
                ans=mid;
                r=mid-1;
            }
