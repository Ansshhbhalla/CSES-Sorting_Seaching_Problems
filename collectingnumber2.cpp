...
int main() {
    Dracarys
        ...
        ll a[n];
        vector<ll> v(n+1);
        for(int i=0;i<n;i++){cin>>a[i]; v[a[i]]=i;}
        vector<int> ans;
        ...
        for(int i=0;i<m;i++){
            ...
            //update counter undo the shit we swapped
            for(auto it:s){
                counter-=v[it.first]>v[it.second];
            }
            ...
            //again update after swap now after swapping we can update the counter accordingly
